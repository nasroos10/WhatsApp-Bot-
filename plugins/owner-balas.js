...
const handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    const caption = `مرحبا @${mentionUser}, لقد تلقيت رسالة من: *المطور*\nالرسالة:\n${pesan}`.trim()
    const mentions = [to]
    ...
    try {
        ...
        if (maybeHasMedia && typeof q.download === 'function') {
            try {
                ...
                if (buf && Buffer.isBuffer(buf)) {
                    const url = await uploadPomf(buf)
                    ...
                    sentWithMedia = true
                    await m.reply('تم إرسال الرسالة بنجاح مع الوسائط.')
                }
            } catch {
                sentWithMedia = false
            }
        ...
        if (!sentWithMedia) {
            ...
            await m.reply('تم إرسال الرسالة بنجاح.')
        }
...
handler.help = ['زاجل']
handler.tags = ['owner']
handler.command = /^(balas|reply|زاجل)$/i
handler.owner = true
handler.fail = null
