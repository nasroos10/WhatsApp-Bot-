...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    // Keep original sendMessage for chaining after we normalize media inputs
    const _origSendMessage = conn.sendMessage.bind(conn)

    let sock = Object.defineProperties(conn, {
        sendMessage: {
            /**
             * Wrapper to normalize media inputs (urls/paths/strings) to Buffers, preventing Baileys from using getHttpStream
             */
                ...
                try {
                    const normalize = async (val) => {
                        ...
                        if (val && typeof val === 'object' && typeof val.url === 'string') {
                            const type = await conn.getFile(val.url, false)
                            return type.data
                        }
                        // Direct string (url or path)
                        if (typeof val === 'string') {
                            const type = await conn.getFile(val, false)
                            return type.data
                        }
                        // Already a Buffer or acceptable stream
                        return val
                    }
                    ...
                    for (const key of mediaKeys) {
                        if (key in msg) {
                            const before = msg[key]
                            const after = await normalize(before)
                            if (after !== before) msg[key] = after
                        }
                    }

                    // Ensure upload handler present for binary uploads
                    const sendOpts = { upload: conn.waUploadToServer, ...options }
                    return await _origSendMessage(jid, msg, sendOpts)
                } catch (e) {
        ...
        getJid: {
            value(sender) {
                // Ensure we always return a normalized JID (xxx@s.whatsapp.net),
                // stripping any device suffix (:
                if (!conn.isLid) conn.isLid = {}
                if (!sender) return sender

                // Decode to strip device part like 628xx:12@s.whatsapp.net
                let s = String(sender).decodeJid()

                // Cached mapping (use decoded key)
                if (conn.isLid[s]) return conn.isLid[s]

                // Already a normalized JID or group/broadcast
                if (/@(s\.whatsapp\.net|g\.us|broadcast)$/.test(s)) return s

                // If it's a bare number, normalize it directly
                if (/^\d+$/.test(s)) return (conn.isLid[s] = `${s}@s.whatsapp.net`)

                // If not a LID, return as-is
                if (!s.endsWith("@lid")) return s

                // Try to resolve LID -> regular JID using known group participants
                for (let chat of Object.values(conn.chats)) {
                    ...
                    if (user) {
                        const resolved = user.id || user.jid || (user.phoneNumber ? `${String(user.phoneNumber).replace(/[^0-9]/g, '')}@s.whatsapp.net` : null)
                        if (resolved) {
                            conn.isLid[s] = resolved.decodeJid ? resolved.decodeJid() : String(resolved).decodeJid()
                            return conn.isLid[s]
                        }
                    }
                }
                // Fallback: leave unchanged (may be resolved later)
                return s
            }
        },
        logger: {
     ...
     * @param {import('@whiskeysockets/baileys').proto.WebMessageInfo} quoted - Message to reply to (optional)
    ...
    async value(jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) {
		...
		return 'document';
            ...
            let message = {
                caption,
                ptt,
                [mtype]: file, // pass Buffer directly to avoid fromWeb on PassThrough
                mimetype,
    ...
    enumerable: true
        ...
        sendSticker: {
            async value(jid, path, quoted, exif = {}) {
                ...
                if (!data || data.length === 0) throw new TypeError('File Tidak Ditemukan')
                // Prefer provided exif, fallback to globals
                const meta = {
     ...
     * @param {import('@whiskeysockets/baileys').proto.WebMessageInfo} quoted - The message to reply to (optional)
...
/**
    ...
    */
     ...
     * @param {import('@whiskeysockets/baileys').proto.WebMessageInfo} message - The message containing media
   ...
   async value (ms) {
    ...
    async value(m, type, saveToFile) {
        try {
            ...
            // Download content from message (Baileys returns an async iterable or a Web ReadableStream depending on env)
            // Workaround: some Baileys versions choke on 'sticker' type with Node PassThrough. Use 'image' for webp stickers.
            const dlType = type === 'sticker' ? 'image' : type;
            let stream;
            try {
                stream = await downloadContentFromMessage(m, dlType);
            } catch (e) {
                // Fallback: if environment complains about ReadableStream vs PassThrough, temporarily patch fromWeb
                if (/readableStream/i.test(String(e?.message)) && /PassThrough/i.test(String(e?.message))) {
                    const nodeStream = await import('stream');
                    const origFromWeb = nodeStream.Readable.fromWeb?.bind(nodeStream.Readable);
                    if (origFromWeb) {
                        try {
                            nodeStream.Readable.fromWeb = function (rs, opts) {
                                // If it's not a WHATWG ReadableStream (no getReader), just return rs as-is
                                if (rs && typeof rs.getReader !== 'function') return rs;
                                return origFromWeb(rs, opts);
                            };
                            stream = await downloadContentFromMessage(m, dlType);
...
export function serialize() {
    // Media message types that support download/processing
    // Added 'stickerMessage' so quoted/download works for stickers
    // (fixes .toimg and .tovideo on replied stickers)
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        sender: {
            get() {
                // Always resolve to the real user JID.
                if (this.key?.fromMe) return (this.conn?.user?.jid || '').decodeJid()
                // Prefer participant fields; fall back to chat (1:1 chats)
                const raw = (this.key?.participantAlt || this.key?.participant || this.chat)
                return this.conn.getJid(String(raw).decodeJid())
            },
            enumerable: true
        },
        fromMe: {
                ...
                return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? {
                    ...
                    mentionedJid: {
                        get() {
                            ...
                            return raw.map(jid => conns.getJid(jid))
                        },
                        enumerable: true
                    },
                    name: {
                    ...
                    download: {
                        value(saveToFile = false) {
                            ...
                            if (!mtype || !mediaRoot || !mediaRoot[mtype]) {
                                throw new Error('No downloadable media found in quoted message. Reply to an image/video/sticker or resend the media.');
                            }
                            return self.conn?.downloadM(mediaRoot[mtype], mtype.replace(/message/i, ''), saveToFile);
                        },
                        enumerable: true,
                        configurable: true,
                    },
        ...
        download: {
            value(saveToFile = false) {
                ...
                const mediaRoot = this.mediaMessage;
                if (!mtype || !mediaRoot || !mediaRoot[mtype]) {
                    throw new Error('No downloadable media found in message. Send or reply to an image/video/sticker.');
                }
                return this.conn?.downloadM(mediaRoot[mtype], mtype.replace(/message/i, ''), saveToFile);
            },
            enumerable: true,
            configurable: true
        },
